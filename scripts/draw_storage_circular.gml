///draw_storage_circular(center x, center y, radius, start angle, percent health, sprite)
...
if ceil(steps*(hp/STORAGE_AVAILABLE)) >= 1 {
...
draw_set_color(c_white)
    ...
    side=!side
    hps=hp/(ceil(steps*(hp/STORAGE_AVAILABLE))+1)
    hpd=0
    repeat ceil(steps*(hp/STORAGE_AVAILABLE)+1) {
        step+=1
        
        if step=ceil(steps*(hp/STORAGE_AVAILABLE)+1) { //final step
